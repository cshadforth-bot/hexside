"""Hexside reference/test AI for the BASE GAME specifically - the same engine and the
same driver logic as driver.py, but playing sim.ROSTER_BASE instead of sim.ROSTER, and
with the 13-card Tactic Deck disabled entirely (never dealt, never refilled - see the
two lines right after the imports below). This is NOT part of the rules engine (see
../engine/README.md) and it is NOT an opponent worth learning from - see driver.py's
own docstring for the general caveats, all of which apply here too.

Why a separate file rather than a flag on driver.py: swapping sim.ROSTER and disabling
Tactic Cards are both simple, load-bearing module-level changes (see right below), and
keeping them as two openly-different files makes it obvious at a glance which ruleset
any given batch result came from, rather than a run-time flag someone could forget to
pass. Imports sim/board from ../engine by path, same as driver.py, so this always tests
whatever is actually canonical. Run it FROM THIS DIRECTORY, not from inside engine/ -
see driver.py's own docstring for why.

Every Tactic Card code path below (all 13, see driver.py's own docstring for the full
list) is still present and unchanged, but none of it can ever fire - hand_tac stays
permanently empty, so every 'X in hand_tac' / can_afford() check below correctly and
safely no-ops on its own. Kept this way deliberately rather than stripped out, so this
file stays a minimal diff from driver.py instead of a second, divergent implementation
to maintain:
...
sim.ROSTER = sim.ROSTER_BASE  # Base Game roster (§14) - every sim.ROSTER[...] lookup
...
# essentially every time - a bounce is locked to one of 8 fixed lines from the carrier's
# own hex, while a straight Pass can target any teammate along 4 axes, so it almost always
# has more angular freedom to find a better-ranked target. This bonus represents the
# trick-play/deception value of going around a defender via the wall, which rank_dist
# alone has no way to price in. rank_dist itself typically runs 1-10 across the board
# (median ~4 to either net) - these are a few units of that scale, tuned to let a bounce
# win close calls without making it win regardless of position.
#
# Split into two separate knobs, not one, because they measured very differently at the
# same value: with a single BOUNCE_RANK_BONUS=2 applied to both tiers, the teammate tier
# landed a healthy real mix (~38% bounce), but the empty-hex tier hit 100% bounce, 0%
# plain Clearance - a bounce-to-empty-hex option is available almost every time that tier
# is even reached (confirmed directly: ~100% of the time on a realistic scattered board),
# so even a modest bonus won it outright, every time. Tried shrinking
# BOUNCE_LOOSE_RANK_BONUS instead of coin-flipping it - didn't work: measured a hard
# switch, not a dial (bonus=0 -> 100% plain Clearance, bonus=1 -> the exact opposite, 100%
# bounce, both across 200 real games, no middle ground) - so this tier's rank_dist gap is
# almost always ~0-1 in real positions, and no fixed threshold can land "a mix" for a
# comparison that close. See the coin-flip at its own call site (in
# try_plain_pass_or_clearance) for how this actually gets a genuine mix instead; see
# [[project_hexside_bounce_pass_mechanic]] for the batch numbers both were calibrated
# against.
BOUNCE_RANK_BONUS = 2
BOUNCE_LOOSE_RANK_BONUS = 2

def _replenish_no_tactics(s, side):
    """Base Game: no Tactic Deck at all - never dealt (see run_match below), never
    refilled here either, so every Tactic Card check throughout this file safely and
    correctly no-ops on its own. Command Cards behave exactly as normal."""
    needed = 5 - len(s['hand_cmd'][side])
    for _ in range(needed):
        c = sim.draw_cmd(s, side)
        if s['debt'][side] > 0:
            s['debt'][side] -= 1
            left = s['debt'][side]
...
sim.replenish = _replenish_no_tactics

def run_match(match_no):
    sim.gen_random()
    s = sim.new_state()
    s['hand_tac'] = {'A': [], 'B': []}  # clear new_state()'s own initial 3-card deal too
    stats = dict(match=match_no, tackles=0, tackles_won=0, dribbles=0, dribbles_won=0,
                 ...
                 bounce_goals=0,
                 # Character-skin Signatures (hexside-character-cards.html, 2026-09-17) -
                 # showman_played stays 0 in this Base-only driver: Nutmeg/Step-Over are
                 # Tactic-deck cards, and Base Game never populates hand_tac at all, so
                 # Showman's trigger can never fire here (not implemented for that reason).
                 ...
                 # Main Character (Me!) redesigned 2026-09-21: the original "must Shoot
                 # instead of Pass when Through on Goal" never had anything to restrict
                 # (this driver never offers that choice to anyone - confirmed dead, not
                 # guessed). Replaced with a real, once/match "reroll ALL Blank dice on a
                 # Shot" - the same mechanism as Clinical/Shot-Stopper above, just sized up.
                 mainchar_played=0,
                 # 'GK' included since 2026-09-17: Playing Out (Sunday Ederson) can put a
                 # Goalkeeper through attempt_shot's normal scoring path, a real code
                 # path this dict didn't previously need to cover - found via a KeyError
                 # crash in exactly that scenario during this feature's own smoke test.
    ...
    def dribble_contest(mover, atk_dice, opp, def_dice, label):
        ...
        if not win and can_afford(mover, 'Nutmeg') and 'flair' in af:
            pay_and_discard(mover, 'Nutmeg')
            stats['nutmeg_played'] += 1
            win = True
            line2 = f"  [{mover} plays Nutmeg (2 Flair) - a Flair symbol wins the Dribble Challenge outright]"
            s['log'].append(line2); print(line2)
        elif not win and can_afford(mover, 'StepOver'):
            pay_and_discard(mover, 'StepOver')
            stats['stepover_played'] += 1
            k = min(2, atk_dice)
            rv, rf, rh = sim.roll(s, mover, k, True)
            ah += rh
            win = ah > dh
            line2 = f"  [{mover} plays Step-Over (1 Flair) - rerolls {k} dice: {rv}->{rf} ({rh} succ)]"
            s['log'].append(line2); print(line2)
        return win, ah, dh
    ...
    def do_tackle(atk_side, atk_unit, def_side, label, use_strong_chance=0.4):
        ...
        if (sim.ROSTER[carrier].get('sig') == 'immovable_object'
            ...
            # carrying target of a Tackle Challenge, +1 Defense die - the engine's
            # contest() doesn't expose raw dice back to callers, so this approximates
            # "cancel one attacker Success" as "add one Stop" instead (same shape as
            # Counter's own die-bonus option, maybe_counter_die above).
        ...
        if (atk_sig == 'no_way_through'
            ...
            # No Way Through (Big Dave): once/match, automatic win, no roll at all - but
            # still draws Discipline TWICE regardless, applying both (identical to a
            # LOST Strong Tackle's penalty, per the card's own text).
        ...
        if sees_red:
            ...
            # Attack die, double Discipline draw on a loss) - free of the Tactic Card/
            # hand requirement, so borrow sim.strong_tackle() itself by handing it the
            # one card it insists on removing from hand, then let it self-remove as
            # normal (Base Game's hand_tac is otherwise always empty).
            ...
            s['hand_tac'][atk_side].append('StrongTackle')
            use_strong = True
        if use_strong:
            if not sees_red:
    ...
    def try_plain_pass_or_clearance(mover, carrier, start):
        ...
        2026-09-16: a bounce-pass-to-teammate (or -to-empty-hex) now competes directly in
        this same candidate pool - NOT treated as a separate, lower-priority fallback the
        way the first version of this mechanic had it (that version left it firing
        essentially never, since this function's own straight-line search almost always
        finds SOMETHING first). A teammate-reaching option (straight or bounced) always
        beats every empty-hex option (straight or bounced) - unchanged from before - the
        bounce/straight split no longer decides the tier by itself. Within a tier, a
        bounce candidate's rank_dist() is discounted by BOUNCE_RANK_BONUS before sorting -
        measured directly that a truly FAIR (undiscounted) comparison makes bounce lose to
        a straight pass almost every time, since a straight Pass can aim at any teammate
        on 4 axes while a bounce is locked to one of 8 fixed lines from the carrier's own
        hex - strictly less freedom to land on the single best-ranked hex. The discount is
        this driver's stand-in for the trick-play value a pure distance metric can't see.
        ...
        if not clear_candidates:
            return None
        # Measured directly (not guessed) that a fixed discount here is a hard switch, not
        # a dial: bonus=0 gave plain Clearance 100% of this tier's decisions, bonus=1
        # flipped to the EXACT opposite, bounce 100%/0 plain - across 200 real games each
        # way, no middle ground. That means the two are almost always tied or a hair apart
        # in real positions, so no fixed threshold can produce a genuine mix - there's no
        # gradient to tune along. Coin-flipping whether the discount applies at all, per
        # decision, is what actually keeps both in play instead of hunting for a
        # deterministic number that doesn't exist for this comparison.
    ...
    def try_reads_game(mover, carrier_unit, carrier_hex):
        ...
        # Flair, no card), same effect as try_jockey below but its own separate
        # once-per-match budget rather than sharing Jockey's Tactic-card one.
    ...
    def move_ball_carrier(mover):
        ...
        if gasser:
            ...
            # (s['half']==2) - not a once-per-match trigger like everything else here,
            # so no 'used' flag, just a standing -1 Pace for the rest of the match.
        ...
        if playing_out_move:
            # Playing Out (Sunday Ederson): once/match, "acts as a normal outfield unit
            # for that one activation" - marked used HERE, the instant the zone-lock
            # bypass is actually granted, not deferred to attempt_shot: a lost Dribble
            # Challenge mid-move (this same function's own ZoC-contest branch, below)
            # can leave the ball loose before attempt_shot ever runs, which would
            # otherwise skip marking this used at all and let it re-fire later.
            #
            # Two separate flags, two separate timescales (2026-09-17, corrected twice
            # after the user's own follow-ups: first that the goalkeeper is forced back
            # "the next turn," not instantly - a real exposure window is the whole
            # point, not a bug to close away entirely - then that the return itself
            # isn't guaranteed either, see play_turn's own comment for that part).
            # playingout_this_turn is THIS activation's own signal (attempt_shot reads
            # it to unlock a Shot too), reset False at the top of every play_turn as
            # usual for one-turn state. playingout_pending_return is NOT cleared next
            # turn - it survives across the opponent's intervening turn (the real risk:
            # the zone is genuinely open the whole time) and is only consumed the next
            # time THIS side's own play_turn runs, which attempts the return.
        ...
        if (carrier == 'SW' and sim.ROSTER['SW'].get('sig') == 'marshalled'
            ...
            # Marshalled (Gaffer): once/match, when activated, one other friendly unit
            # may act as though it had also been activated this turn - simplified here
            # by reusing Full Press's own existing "second Act if the ball ends up with
            # a different unit" mechanism (followup_move_then_maybe_shoot) rather than
            # a genuine independent second activation, matching this driver's existing
            # precedent that Full Press itself is ALREADY simplified the same way (see
            # its own docstring). A scoped-down "counts as this turn's Full Press
            # bonus," not a literal second full Act with its own Tackle/Pass options.
        ...
        if best_hex == start:
            ...
            # try_plain_pass_or_clearance itself now, competing directly against the
            # straight-line options there by the same rank_dist() ranking.
            if not has_shot and try_bounce_shot(mover, carrier, start):
                play_card(mover, card, f"activates {carrier}")
                return
            # Main Character (Me!) has NO effect to add here, on inspection rather than
            # assumption: breakaway['flag'] is only ever set inside this same function's
            # OWN Dribble Challenge branch below (best_hex != start), which is mutually
            # exclusive with this best_hex==start branch and always returns before a
            # Pass/Clearance decision is reached. In other words, this driver already
            # never offers a Through-on-Goal unit a Pass instead of a Shot, for ANY
            # character - Me!'s restriction has nothing left to override here. An
            # earlier version of this code checked breakaway['flag'] at this exact spot
            # believing it could be True here; it never was, confirmed by a 0/60 stat
            # across a dedicated smoke test, then by tracing every breakaway['flag']
            # assignment directly rather than continuing to guess. Removed rather than
        ...
        if (contested_hex is not None and carrier_sig in ('one_more_run', 'out_of_nowhere')
            ...
            # One More Run (Dodgy Knee) / Out of Nowhere (Flash): once/match, this
            # unit's own move ignores the opposing Zone of Control entirely - no
            # Dribble Challenge. Only spends the once-per-match charge when there was
            # actually a ZoC to push through (checked above), not on an uncontested
            # move that had nothing to bypass. Shared 'used' flag since a unit carries
            # only one of the two Signatures at a time.
    ...
    def attempt_shot(mover):
        ...
        if s.setdefault('outofnowhere_no_shot', {'A':False,'B':False})[mover]:
            # Out of Nowhere (Flash): the "no Shot on that same activation" half of the
            # once-per-match ZoC-ignoring move set in move_ball_carrier - consumed here,
            # the instant this Act's own Shot attempt would otherwise happen, then
            # cleared so it doesn't linger into a later turn.
        ...
        if srange is None and s.setdefault('playingout_this_turn', {'A':False,'B':False})[mover]:
            # Playing Out (Sunday Ederson): the one concrete rules difference that
            # matters for a Shot is srange itself (None is what makes a Goalkeeper
            # unable to Shoot at all) - grant it WB's own Shot Range (2) for this
            # attempt only. 'used'/logging/stats already happened in move_ball_carrier,
            # the instant the zone-lock bypass was granted - this just reads that same
            # per-turn flag rather than re-deriving or re-marking anything.
            srange = 2
        ...
        if (not win and sim.ROSTER[carrier].get('sig') == 'main_character'
            ...
            # just Clinical's one) - "he never misses the big moment." Same shape as
            # Clinical/Shot-Stopper (roll N fresh dice rather than re-rolling the same
            # physical die), sized to however many blanks the original roll actually
            # had. Marked used on any lost-Shot attempt while eligible, same as
            # Clinical, even on the rare roll with zero blanks to reroll.
        ...
        if guard_sig in ('shot_stopper', 'immovable') and not s.setdefault('shotstopper_used', {'A':False,'B':False})[opp]:
            ...
            # once per match, free - the real Goalkeeper only, never an Emergency
            # deputy (same restriction the retired Command the Box had), and never
            # "Playing Out" (its Signature is about acting as an outfield unit, not a
            # better Save). Immovable rerolls up to 2 dice instead of Shot-Stopper's 1 -
            # the one purely numeric upgrade among the character skins. Checked
            # unconditionally rather than gated on win/lose, because it helps either
            # way: a Stop can still flip a losing Save into a won one here, and even
            # when the Save's already won, the extra die(s) feed the Flair-retention
            # check below just like the original roll would. Same simplification as
            # Clinical above: doesn't verify a Blank was actually rolled, just adds the
            # bonus die and keeps it if it helps - equivalent in effect, since a die
            # that already hit or flaired would never be the one worth rerolling anyway.
    ...
    def resolve_bounce_net(mover, carrier, start):
        ...
        of range here - that's why a bounce was needed at all). Deliberately skips
        attempt_shot's Composure/Curl Shot checks - both are Tactic Cards, and this
        driver's hand_tac is always empty (see its own module docstring), so those checks
        are dead code here regardless. Clinical and Shot-Stopper (Signatures, not Tactic
        Cards) ARE replayed, since those apply in the Base Game too. Rebound placement
        reuses B.lane() on the PASSER's own hex (`start`) as the closest real substitute
        for "which lane did this shot effectively come from" - a bounced path can cross
        both lanes along its bend, and the passer's own position is the least arbitrary
        single answer available, the same spirit as attempt_shot's own seam-row tiebreak."""
        end = att_end(mover)
        opp = other(mover)
        ...
        label = f"Bounce Pass -> Shot: {mover} {carrier} vs {opp} {guard}"
        av, af, ah = sim.roll(s, mover, shot_dice, True)
        dv, df, dh = sim.roll(s, opp, save, False)
        win = ah > dh
        line = (f"[{label}] {mover} rolls {av}->{af} ({ah} succ) | {opp} rolls {dv}->{df} "
        ...
        if (not win and sim.ROSTER[carrier].get('sig') == 'clinical'
            ...
            s['clinical_used'][mover] = True
            stats['clinical_played'] += 1
            s['log'].append(f"  [{mover} PO Clinical (§14) - reroll one Blank die on the Shot, once per match, free]"); print(s['log'][-1])
            rv, rf, rh = sim.roll(s, mover, 1, True)
            if rf[0] == 'success':
                ah += 1
                win = ah > dh
        if (not win and sim.ROSTER[carrier].get('sig') == 'main_character'
            ...
            n_blank = af.count('blank')
            if n_blank:
                s['log'].append(f"  [{mover} PO Main Character - reroll all {n_blank} Blank dice on the Shot, once per match, free]"); print(s['log'][-1])
                rv, rf, rh = sim.roll(s, mover, n_blank, True)
                ah += rh
                win = ah > dh
        guard_sig = sim.ROSTER[guard].get('sig') if guard == 'GK' else None
    ...
    def try_bounce_shot(mover, carrier, start):
        ...
        check just missed, not a fallback. The OTHER half - bounce-to-teammate and
        bounce-to-empty-hex - lives inside try_plain_pass_or_clearance itself now, ranked
        directly against the straight-line options there by the same rank_dist(), rather
        than being checked separately here - an earlier version had it as a lower-priority
        fallback checked only after that function already ran, which left it firing
        essentially never (that function almost always finds SOMETHING first). Returns
        ...
        # srange). Without this check a GK carrying the ball could back-door a "shot" a
        # normal Shot could never take - not a real rules gap, just this mechanic needing
        # the same guard.
        can_shoot = sim.ROSTER[carrier]['srange'] is not None
        # bounce_pass_options reports a 'net' outcome for EITHER end reached, own goal
        # included - it has no notion of attacker/defender, that's this driver's job, the
        # same way B.shot_dist() itself is symmetric and has_shot is what supplies the
        # direction. Filtering to att_end(mover) here is load-bearing, not a style choice:
        # without it a bounce line toward the mover's OWN net would still trigger a full
        # Save-contested "shot" resolved against the opponent's goal (resolve_bounce_net
        # always resolves against att_end internally), which is nonsense - wasting the
        # turn on a geometric coincidence pointed the wrong way entirely.
        end = att_end(mover)
    ...
    def resolve_bounce_completion(mover, carrier, start, kind, target, recv):
        ...
        is fixed the instant it's declared (it has to follow the wall it reflects off),
        so there's nothing analogous to contest - the whole path resolves at once, same
        spirit as a Long Ball ignoring intervening Zones of Control, just for a bent line
    ...
    def play_turn(turn_no, mover):
        ...
        if s.setdefault('playingout_pending_return', {'A':False,'B':False})[mover] and 'GK' in s['pos'][mover]:
            # Playing Out (Sunday Ederson): at the start of this side's own next turn,
            # the Goalkeeper must get back into its own zone under its own normal Pace -
            # a real move, blockable by units on EITHER side, not a magic teleport
            # (2026-09-17, per the user's own correction to an earlier, too-safe
            # version of this fix). legal_reach's own zone-lock filter already does
            # exactly the right check for free: for a goalkeeper-role unit it only ever
            # returns hexes that are BOTH reachable this turn AND inside the zone, so
            # "can it get back at all" is just "is this non-empty."
            #
            # If blocked or Pace falls short (the common case - confirmed directly
            # that this is usually empty once the unit is outside its own Pace range of
            # the zone, not a rare edge case): no free pass. A guaranteed Yellow Card
            # (not a random Discipline Deck draw - the user asked for this outcome
            # specifically, not a chance of one) applied directly via
            # apply_discipline(), which already does everything else needed with no
            # new code: permanent Pace -1, and send_to_sinbin() pulls it off the pitch
            # for one turn - during which the zone is genuinely empty too, "real
            # tension by design" per that function's own docstring, same as any other
            # sin-binned goalkeeper. sinbin_check() (called every turn, right below)
            # already returns a goalkeeper-role unit straight into its zone the instant
            # it comes back, so playingout_pending_return's own job ends right here -
            # the sin-bin's existing lifecycle owns the rest from this point on. A
            # SECOND failure (this unit's already-carried Yellow) escalates to Red via
            # apply_discipline's own existing logic, sending the real Goalkeeper off
            # entirely and triggering Emergency Goalkeeper assignment automatically -
            # an appropriately severe, fully organic consequence for this card's own
            # "occasionally undoes the whole team" flavor, not something added on top.
